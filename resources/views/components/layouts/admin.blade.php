...
<body class="bg-[#0f0f1a] text-white antialiased" x-data="{ sidebarOpen: false }">
    <div class="flex min-h-screen">
        ...
        <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-[#0a0a12] border-r border-white/10 transform transition-transform lg:translate-x-0"
            ...
            <nav class="p-4 space-y-1">
                <a href="#" 
                   class="flex items-center gap-3 px-4 py-3 rounded-xl transition-colors bg-[#e50914] text-white">
                ...
                <div class="h-full px-4 lg:px-8 flex items-center justify-between">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open" class="flex items-center gap-2 text-sm text-gray-300 hover:text-white">
                            <img src="https://ui-avatars.com/api/?name=Admin+User&background=e50914&color=fff" alt="" class="w-8 h-8 rounded-full">
    ...
    <!-- Global Confirm Modal (Dummy) -->
    <div x-data="{ open: false }" x-show="open" class="fixed inset-0 z-50 flex items-center justify-center" style="display: none;">
        <div class="fixed inset-0 bg-black/50"></div>
        <div class="relative bg-[#16162a] rounded-xl p-6 max-w-md w-full mx-4 border border-white/10">
            <h3 class="text-lg font-semibold text-white mb-2">Konfirmasi Hapus</h3>
            <p class="text-gray-400 mb-4">Data yang dihapus tidak dapat dikembalikan. Yakin ingin melanjutkan?</p>
            <div class="flex justify-end gap-3">
                <button @click="open = false" class="px-4 py-2 text-gray-400 hover:text-white">Batal</button>
                <button class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">Ya, Hapus</button>
            </div>
        </div>
    </div>
