...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <div class="bg-[#16162a] rounded-xl border border-white/10 p-3 sm:p-4">
            <p class="text-gray-400 text-xs sm:text-sm">Total Booking</p>
            <p class="text-xl sm:text-2xl font-bold text-white mt-1">{{ number_format($summary['total_bookings']) }}</p>
        </div>
        <div class="bg-[#16162a] rounded-xl border border-white/10 p-3 sm:p-4">
            <p class="text-gray-400 text-xs sm:text-sm">Total Tiket</p>
            <p class="text-xl sm:text-2xl font-bold text-white mt-1">{{ number_format($summary['total_tickets']) }}</p>
        </div>
        <div class="bg-[#16162a] rounded-xl border border-white/10 p-3 sm:p-4">
            <p class="text-gray-400 text-xs sm:text-sm">Total Pendapatan</p>
            <p class="text-lg sm:text-2xl font-bold text-green-500 mt-1">Rp {{ number_format($summary['total_revenue'], 0, ',', '.') }}</p>
        </div>
        <div class="bg-[#16162a] rounded-xl border border-white/10 p-3 sm:p-4">
            <p class="text-gray-400 text-xs sm:text-sm">Rata-rata Order</p>
            <p class="text-lg sm:text-2xl font-bold text-white mt-1">Rp {{ number_format($summary['avg_order_value'], 0, ',', '.') }}</p>
        </div>
    </div>
    ...
    <div class="bg-[#16162a] rounded-xl border border-white/10 p-4 sm:p-6">
        ...
        <div class="mobile-cards space-y-3">
            ...
            <div class="mobile-card">
                <h4 class="text-white font-medium mb-2">{{ $item['movie'] }}</h4>
                <div class="mobile-card-row">
                    <span class="mobile-card-label">Booking</span>
                    <span class="mobile-card-value">{{ $item['bookings'] }}</span>
                </div>
                <div class="mobile-card-row">
                    <span class="mobile-card-label">Tiket</span>
                    <span class="mobile-card-value">{{ $item['tickets'] }}</span>
                </div>
                <div class="mobile-card-row">
                    <span class="mobile-card-label">Pendapatan</span>
                    <span class="mobile-card-value text-green-500 font-medium">Rp {{ number_format($item['revenue'], 0, ',', '.') }}</span>
                </div>
            </div>
            @empty
            <div class="text-center py-4 text-gray-400">Tidak ada data</div>
            @endforelse
        </div>
    </div>
    ...
    <div class="bg-[#16162a] rounded-xl border border-white/10 p-4 sm:p-6">
        ...
        <div class="mobile-cards space-y-3">
            @forelse ($revenueByDate as $item)
            <div class="mobile-card">
                <h4 class="text-white font-medium mb-2">{{ $item['date'] }}</h4>
                <div class="mobile-card-row">
                    <span class="mobile-card-label">Booking</span>
                    <span class="mobile-card-value">{{ $item['bookings'] }}</span>
                </div>
        ...
        <div class="mobile-cards space-y-3">
            ...
            <div class="mobile-card">
                <div class="flex items-center justify-between mb-3">
                    <span class="text-white font-mono text-sm bg-white/10 px-2 py-1 rounded">{{ $booking->booking_code }}</span>
                    <span class="text-green-500 font-semibold text-sm">Rp {{ number_format($booking->total_price, 0, ',', '.') }}</span>
                </div>
                <h4 class="text-white font-medium mb-1">{{ Str::limit($booking->showtime->movie->title, 30) }}</h4>
                <p class="text-gray-400 text-sm mb-2">{{ $booking->user->name }}</p>
                <div class="mobile-card-row">
                    <span class="mobile-card-label">Bioskop</span>
                    <span class="mobile-card-value text-sm">{{ $booking->showtime->studio->cinema->name }}</span>
                </div>
                <div class="mobile-card-row">
                    <span class="mobile-card-label">Tiket</span>
                    <span class="mobile-card-value">{{ $booking->total_seats }}</span>
                </div>
                <div class="mobile-card-row">
                    <span class="mobile-card-label">Waktu Bayar</span>
                    <span class="mobile-card-value text-sm">{{ $booking->paid_at->format('d M Y H:i') }}</span>
                </div>
            </div>
            @empty
            <div class="text-center py-8 text-gray-400">Tidak ada data booking</div>
            @endforelse
        </div>
    </div>
</div>
