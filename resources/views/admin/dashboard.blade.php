...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 mb-6 sm:mb-8">
    <div class="bg-[#16162a] rounded-xl p-4 sm:p-6 border border-white/10">
        <div class="flex items-center justify-between">
            <div class="flex-1 min-w-0">
                ...
                <p class="text-2xl sm:text-3xl font-bold text-white mt-1">{{ $stats['total_movies'] }}</p>
                <p class="text-xs sm:text-sm text-gray-500 mt-1">{{ $stats['now_playing'] }} sedang tayang</p>
            </div>
    ...
    <div class="bg-[#16162a] rounded-xl p-4 sm:p-6 border border-white/10">
        <div class="flex items-center justify-between">
            <div class="flex-1 min-w-0">
                ...
                <p class="text-lg sm:text-2xl font-bold text-white mt-1">Rp {{ number_format($stats['total_revenue'], 0, ',', '.') }}</p>
                <p class="text-xs sm:text-sm text-green-400 mt-1 truncate">+Rp {{ number_format($stats['today_revenue'], 0, ',', '.') }}</p>
            </div>
    ...
    <div class="bg-[#16162a] rounded-xl p-4 sm:p-6 border border-white/10">
        <div class="flex items-center justify-between">
            ...
            <div class="w-10 h-10 sm:w-12 sm:h-12 bg-yellow-500/20 rounded-xl flex items-center justify-center flex-shrink-0 ml-2">
    ...
    <div class="lg:col-span-2 bg-[#16162a] rounded-xl border border-white/10">
        <div class="p-4 sm:p-6 border-b border-white/10">
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('admin.bookings.index') }}" class="text-xs sm:text-sm text-[#e50914] hover:underline">Lihat Semua</a>
            </div>
        </div>
            ...
            @forelse($recentBookings as $booking)
                <div class="mobile-card">
                    <div class="flex items-center justify-between mb-2">
                        <span class="font-mono text-xs text-white bg-white/10 px-2 py-1 rounded">{{ $booking->booking_code }}</span>
                        @php $status = $booking->status_label; @endphp
                        <span class="px-2 py-1 rounded-full text-xs font-medium
                            @if($status['color'] === 'green') bg-green-500/20 text-green-400
                            @elseif($status['color'] === 'yellow') bg-yellow-500/20 text-yellow-400
                    ...
                    <p class="text-white font-medium text-sm">{{ Str::limit($booking->showtime->movie->title, 30) }}</p>
                    <p class="text-gray-400 text-xs mb-2">{{ $booking->user->name }}</p>
                    <div class="flex items-center justify-between pt-2 border-t border-white/10">
                        <span class="text-gray-400 text-xs">Total</span>
                        <span class="text-green-400 font-semibold text-sm">{{ $booking->formatted_price }}</span>
                    </div>
                </div>
            @empty
        ...
        <div class="p-4 sm:p-6 space-y-4">
            @forelse($popularMovies as $index => $movie)
                <div class="flex items-center gap-3 sm:gap-4">
                    ...
                    <img src="{{ $movie->poster_url }}" alt="" class="w-10 h-14 sm:w-12 sm:h-16 object-cover rounded flex-shrink-0">
                    <div class="flex-1 min-w-0">
                        <p class="text-white font-medium text-sm truncate">{{ $movie->title }}</p>
                        <p class="text-xs sm:text-sm text-gray-400">{{ $movie->showtimes_count ?? 0 }} pemesanan</p>
                    </div>
                </div>
            @empty
    ...
    <div class="h-48 sm:h-64 flex items-end justify-between gap-1 sm:gap-4 pb-6 sm:pb-8">
        @foreach($revenueData as $data)
            ...
            <div class="flex-1 flex flex-col items-center gap-1 sm:gap-2">
                ...
                <span class="text-[10px] sm:text-xs text-gray-400">{{ $data['date'] }}</span>
            </div>
        @endforeach
    </div>
</div>
