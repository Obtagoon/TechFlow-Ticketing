            ...
            <tr class="hover:bg-white/5">
                <td class="px-6 py-4">
                    <div class="flex items-center gap-3">
                        <img src="{{ $movie->poster_url }}" alt="" class="w-10 h-14 object-cover rounded">
                        <div>
                            <p class="text-white font-medium">{{ $movie->title }}</p>
                            @if($movie->release_date)
                                <p class="text-sm text-gray-500">{{ $movie->release_date->format('d M Y') }}</p>
                            @endif
                        </div>
                    </div>
                </td>
                <td class="px-6 py-4 text-gray-300">{{ Str::limit($movie->genre, 20) }}</td>
                ...
                <td class="px-6 py-4">
                    <div class="flex items-center gap-1">
                        ...
                        <span class="text-white">{{ number_format($movie->rating, 1) }}</span>
                    </div>
                </td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4">
                    <div class="flex items-center gap-2">
                        <a href="{{ route('admin.movies.edit', $movie) }}" class="p-2 text-gray-400 hover:text-white transition-colors">
                        ...
                        <button type="button" @click="confirmModal('delete-movie-{{ $movie->id }}')" class="p-2 text-gray-400 hover:text-red-400 transition-colors">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
