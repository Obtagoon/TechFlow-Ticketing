...
<div class="mobile-cards">
    <div class="space-y-3 p-1">
        @forelse($studios as $studio)
            <div class="mobile-card">
                <div class="flex items-center justify-between mb-3">
                    <h3 class="text-white font-semibold">{{ $studio->name }}</h3>
                    <span class="px-2 py-1 bg-purple-500/20 text-purple-400 text-xs rounded-full">
                        {{ $studio->type_label }}
                    </span>
                </div>
                <div class="mobile-card-row">
                    ...
                    <span class="mobile-card-value">{{ $studio->cinema->name }}</span>
                </div>
                <div class="mobile-card-row">
                    <span class="mobile-card-label">Kapasitas</span>
                    <span class="mobile-card-value">{{ $studio->capacity }} kursi</span>
                </div>
                <div class="mobile-card-row">
                    <span class="mobile-card-label">Layout</span>
                    <span class="mobile-card-value">{{ $studio->rows }} x {{ $studio->seats_per_row }}</span>
                </div>
                <div class="mobile-card-row">
                ...
                <div class="flex items-center gap-2 mt-3 pt-3 border-t border-white/10">
                    <a href="{{ route('admin.studios.edit', $studio) }}" class="flex-1 px-3 py-2 bg-white/10 text-white text-center text-sm rounded-lg hover:bg-white/20">
                        Edit
                    </a>
                    <form id="delete-studio-mobile-{{ $studio->id }}" action="{{ route('admin.studios.destroy', $studio) }}" method="POST" class="flex-1">
                        @csrf
                        @method('DELETE')
                        <button type="button" @click="confirmModal('delete-studio-mobile-{{ $studio->id }}')" class="w-full px-3 py-2 bg-red-500/20 text-red-400 text-sm rounded-lg hover:bg-red-500/30">
                            Hapus
                        </button>
                    </form>
                </div>
        ...
        @empty
            <div class="text-center py-12 text-gray-500">Belum ada studio</div>
        @endforelse
    </div>
    @if($studios->hasPages())
        <div class="mt-4">
