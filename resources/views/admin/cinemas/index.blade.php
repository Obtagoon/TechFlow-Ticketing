...
<div class="mobile-cards">
    <div class="space-y-3 p-1">
        @forelse($cinemas as $cinema)
            <div class="mobile-card">
                <div class="flex items-start gap-3 mb-3">
                    @if($cinema->image)
                        <img src="{{ $cinema->image_url }}" alt="" class="w-14 h-14 object-cover rounded-lg">
                    @else
                    ...
                    @endif
                    <div class="flex-1">
                        <h3 class="text-white font-semibold">{{ $cinema->name }}</h3>
                        <p class="text-gray-400 text-sm">{{ Str::limit($cinema->address, 40) }}</p>
                    </div>
                </div>
                <div class="mobile-card-row">
                    <span class="mobile-card-label">Kota</span>
                    <span class="mobile-card-value">{{ $cinema->city }}</span>
                </div>
                <div class="mobile-card-row">
                    <span class="mobile-card-label">Telepon</span>
                    <span class="mobile-card-value">{{ $cinema->phone ?? '-' }}</span>
                </div>
                <div class="mobile-card-row">
                    <span class="mobile-card-label">Studio</span>
                    <span class="mobile-card-value">{{ $cinema->studios->count() }} studio</span>
                </div>
                <div class="mobile-card-row">
                ...
                <div class="flex items-center gap-2 mt-3 pt-3 border-t border-white/10">
                    <a href="{{ route('admin.cinemas.edit', $cinema) }}" class="flex-1 px-3 py-2 bg-white/10 text-white text-center text-sm rounded-lg hover:bg-white/20">
                        Edit
                    </a>
                    <form id="delete-cinema-mobile-{{ $cinema->id }}" action="{{ route('admin.cinemas.destroy', $cinema) }}" method="POST" class="flex-1">
                        @csrf
                        @method('DELETE')
                        <button type="button" @click="confirmModal('delete-cinema-mobile-{{ $cinema->id }}')" class="w-full px-3 py-2 bg-red-500/20 text-red-400 text-sm rounded-lg hover:bg-red-500/30">
                            Hapus
                        </button>
                    </form>
                </div>
        ...
        @empty
            <div class="text-center py-12 text-gray-500">Belum ada bioskop</div>
        @endforelse
    </div>
    @if($cinemas->hasPages())
        <div class="mt-4">
